...
through ten visual stages from seed to full bloom. Petals from the blooms buy paths, pots and
ornaments, so the plot is designed as well as filled. A client-only PWA (React + Vite, no backend,
no account) living entirely under `web/`, with its game data in `docs/*.csv`.
...
a gnome that auto-tends the chosen bed at the same capped rate, and **decoration** (paths, fences,
pots, ornaments) which does nothing to growth at all. **Two seeds a day**; clearing is unlimited and
loses an unbloomed plant, while decoration always comes back to the shed.
...
- **`src/game/`** — pure logic, no React and no I/O: `config` (the live config store),
  `plants` (CSV → species), `decor` (CSV → ornaments), `catalog` (derived tables for both, rebuilt on
  every config change — never cache its arrays across one), `tuning` (knobs CSV + `DEFAULT_KNOBS` +
  the stage-cost curve), `garden` (the grid: sow/clear/harvest, and the two decoration slots per
  cell), `growth` (units → stages → blooms), `daily` (day clock, planting allowance), `inventory`,
  `shop` (petals, including the decoration aisle), `throttle`, `pedometer`, `balanceHarness`.
- **`src/components/`** — `GardenField` (the CSS-3D rotatable plot), `PlantSprite` and `DecorSprite`
  (every species at every stage, and every ornament, drawn from a CSV row — no art assets),
  `SeedTray`, `DecorTray`, `BedPanel`, `ShedPanel`, `WaterButton`, `Almanac`, `Onboarding`,
...
`docs/plants.csv` (species), `docs/decor.csv` (ornaments) and a tuning CSV (`key,value`) are read at
runtime by `game/config.ts`. The **⚙ Data** panel downloads what the game is running on, takes an
upload or a paste, applies it immediately and persists it (`gd.config.v1`) until reverted. Applying
anything **re-makes the garden** (`resetGame()` → wipes `gd.save.v1`, emits `world_reset`) because
species ids, grid size and stage count all move. An unreadable species file is rejected and the
running config kept; an unreadable decor file only costs the ornaments, since decoration is
cosmetic. Softer problems surface as warnings in the panel.
...
Code-side: `game/shop.ts` (petal prices for seeds, fertilizer and gnomes — ornaments are priced in
`decor.csv`), `game/pedometer.ts` (detection thresholds). After any
